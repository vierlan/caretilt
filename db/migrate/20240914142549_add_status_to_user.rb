class AddStatusToUser < ActiveRecord::Migration[7.2]
  def up
    ...
    User.where(verified: false).update_all(status: 0)  # Set status to 'not_verified' (0) where verified is false
  ...
  def down
    ...
    User.where(status: 0).update_all(verified: false)  # Set verified to false where status is 'not_verified' (0)
